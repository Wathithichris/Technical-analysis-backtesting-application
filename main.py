...
if strategy == "SMA crossover":
    sma_short = int(st.number_input("Sma short", key='sma1', format='%f'))
    sma_long = int(st.number_input("Sma long", key='sma2', format="%f"))

    if sma_short != sma_long:
        if sma_short >= sma_long:
            st.warning("sma short must be less than sma long", icon=":material/error:")
    else:
        st.warning("The simple moving averages cannot be equal", icon=":material/error:")
...
if backtest:
    ...
    if strategy=="SMA crossover":
     data = Returns(ticker=ticker, start_date=start_date, end_date=end_date, strategy=strategy,
                       sma_long=sma_long, sma_short=sma_short)
    ...
    try:
        ...
        st.download_button(label="Download results csv", data=results,
